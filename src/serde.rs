...
/// ```
impl<const SIZE: usize, const M: usize> ser::Serialize for CidGeneric<SIZE, M> {
...
pub struct BytesToCidVisitor<const SIZE: usize, const M: usize>;

impl<'de, const SIZE: usize, const M: usize> de::Visitor<'de>
  for BytesToCidVisitor<SIZE, M>
{
  type Value = CidGeneric<SIZE, M>;
  ...
  where
    E: de::Error,
  {
    CidGeneric::<SIZE, M>::try_from(value).map_err(|err| {
    ...
    A: de::SeqAccess<'de>,
  {
    ...
    CidGeneric::<SIZE, M>::try_from(bytes).map_err(|err| {
      de::Error::custom(format!("Failed to deserialize CID: {}", err))
    })
  }
}
...
/// struct $__private__serde__identifier__for__cid(serde_bytes::BytesBuf);
/// ```
impl<'de, const SIZE: usize, const M: usize> de::Deserialize<'de>
  for CidGeneric<SIZE, M>
{
    ...
    D: de::Deserializer<'de>,
  {
    ...
    /// to bytes.
    struct MainEntryVisitor<const SIZE: usize, const M: usize>;

    impl<'de, const SIZE: usize, const M: usize> de::Visitor<'de>
      for MainEntryVisitor<SIZE, M>
    {
      type Value = CidGeneric<SIZE, M>;
...
mod tests {
  ...
  #[test]
  fn test_cid_serde() {
    let cid = CidGeneric::<70, 0>::try_from(
