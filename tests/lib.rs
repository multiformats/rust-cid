...
fn basic_marshalling() {
    let h = multihash::encode(multihash::Hash::SHA2256, b"beep boop")
        .unwrap()
        .into_bytes();
...
fn prefix_roundtrip() {
    ...
    let h = multihash::encode(multihash::Hash::SHA2256, data)
        .unwrap()
        .into_bytes();
...
fn test_base32() {
    ...
    assert_eq!(
        cid.hash,
        multihash::encode(multihash::Hash::SHA2256, b"foo")
            .unwrap()
            .into_bytes()
    );
}
